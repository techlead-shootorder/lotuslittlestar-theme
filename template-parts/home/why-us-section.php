<?php
/**
 * Template part for displaying the Why Choose Us section
 *
 * @package Lotus
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}
?>

<section class="py-14 bg-brand-bg/50 border-b border-brand-cream/60 relative overflow-hidden">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		
		<!-- Section Header -->
		<div class="text-center max-w-3xl mx-auto mb-16">
			<h2 class="text-3xl sm:text-4xl font-bold text-brand-green font-outfit">
				Why Choose Lotus Little Stars Hospital?
			</h2>
		</div>

		<!-- Cards Grid -->
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8">
			
			<!-- Card 1: Trust & Excellence -->
			<div class="bg-white rounded-[2rem] border border-[#EBE8E2] shadow-sm hover:shadow-md transition-all duration-300 p-8 flex flex-col items-start text-left md:col-span-1 lg:col-span-2 group">
				<div class="w-12 h-12 flex items-center justify-center rounded-2xl bg-brand-red/10 text-brand-red mb-6 group-hover:scale-105 transition-transform duration-300">
					<!-- Checkmark Seal SVG -->
					<svg class="w-6 h-6 text-brand-red" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M12 2L15 5L19 4L18 8L21 11L18 14L19 18L15 17L12 20L9 17L5 18L6 14L3 11L6 8L5 4L9 5Z" />
						<path d="m9 11.5 2 2 4-4" />
					</svg>
				</div>
				<h3 class="text-lg md:text-xl font-bold text-brand-green mb-3 group-hover:text-brand-red transition-colors font-outfit">
					A Tradition of Trust and Excellence
				</h3>
				<p class="text-sm text-brand-muted leading-relaxed font-sans">
					A legacy of 15+ years delivering trusted, compassionate, and clinically excellent care for women and children.
				</p>
			</div>

			<!-- Card 2: Integrated Care -->
			<div class="bg-white rounded-[2rem] border border-[#EBE8E2] shadow-sm hover:shadow-md transition-all duration-300 p-8 flex flex-col items-start text-left md:col-span-1 lg:col-span-2 group">
				<div class="w-12 h-12 flex items-center justify-center rounded-2xl bg-brand-red/10 text-brand-red mb-6 group-hover:scale-105 transition-transform duration-300">
					<!-- Infinity SVG -->
					<svg class="w-6 h-6 text-brand-red" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M12 12c-2-2.67-4-4-6-4a4 4 0 1 0 0 8c2 0 4-1.33 6-4Zm0 0c2 2.67 4 4 6 4a4 4 0 1 0 0-8c-2 0-4 1.33-6 4Z"/>
					</svg>
				</div>
				<h3 class="text-lg md:text-xl font-bold text-brand-green mb-3 group-hover:text-brand-red transition-colors font-outfit">
					Integrated Care for Women and Children
				</h3>
				<p class="text-sm text-brand-muted leading-relaxed font-sans">
					Comprehensive, end-to-end care across every stage of life, delivered seamlessly under one roof.
				</p>
			</div>

			<!-- Card 3: Leading Specialists -->
			<div class="bg-white rounded-[2rem] border border-[#EBE8E2] shadow-sm hover:shadow-md transition-all duration-300 p-8 flex flex-col items-start text-left md:col-span-1 lg:col-span-2 group">
				<div class="w-12 h-12 flex items-center justify-center rounded-2xl bg-brand-red/10 text-brand-red mb-6 group-hover:scale-105 transition-transform duration-300">
					<!-- Group of Users SVG -->
					<svg class="w-6 h-6 text-brand-red" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
						<circle cx="9" cy="7" r="4" />
						<path d="M22 21v-2a4 4 0 0 0-3-3.87" />
						<path d="M16 3.13a4 4 0 0 1 0 7.75" />
					</svg>
				</div>
				<h3 class="text-lg md:text-xl font-bold text-brand-green mb-3 group-hover:text-brand-red transition-colors font-outfit">
					Leading Specialists, Exceptional Care
				</h3>
				<p class="text-sm text-brand-muted leading-relaxed font-sans">
					Expert-led, evidence-based care delivered by highly experienced specialists committed to superior outcomes.
				</p>
			</div>

			<!-- Card 4: Expert Guidance -->
			<div class="bg-white rounded-[2rem] border border-[#EBE8E2] shadow-sm hover:shadow-md transition-all duration-300 p-8 flex flex-col items-start text-left md:col-span-1 lg:col-span-2 lg:col-start-2 group">
				<div class="w-12 h-12 flex items-center justify-center rounded-2xl bg-brand-red/10 text-brand-red mb-6 group-hover:scale-105 transition-transform duration-300">
					<!-- Abstract Brain/Mind SVG -->
					<svg class="w-6 h-6 text-brand-red" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96-.44 2.5 2.5 0 0 1 0-3.12 3 3 0 0 1 0-4.88 2.5 2.5 0 0 1 0-3.12A2.5 2.5 0 0 1 9.5 2Z" />
						<path d="M14.5 2A2.5 2.5 0 0 0 12 4.5v15a2.5 2.5 0 0 0 4.96-.44 2.5 2.5 0 0 0 0-3.12 3 3 0 0 0 0-4.88 2.5 2.5 0 0 0 0-3.12A2.5 2.5 0 0 0 14.5 2Z" />
					</svg>
				</div>
				<h3 class="text-lg md:text-xl font-bold text-brand-green mb-3 group-hover:text-brand-red transition-colors font-outfit">
					Expert Guidance Beyond Boundaries
				</h3>
				<p class="text-sm text-brand-muted leading-relaxed font-sans">
					Specialist consultations and second opinions that empower confident, informed healthcare decisions.
				</p>
			</div>

			<!-- Card 5: Growing Network -->
			<div class="bg-white rounded-[2rem] border border-[#EBE8E2] shadow-sm hover:shadow-md transition-all duration-300 p-8 flex flex-col items-start text-left md:col-span-2 lg:col-span-2 group">
				<div class="w-12 h-12 flex items-center justify-center rounded-2xl bg-brand-red/10 text-brand-red mb-6 group-hover:scale-105 transition-transform duration-300">
					<!-- Connected Nodes SVG -->
					<svg class="w-6 h-6 text-brand-red" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<circle cx="12" cy="12" r="3" />
						<path d="M12 9V3m0 18v-6M9 12H3m18 0h-6M5.5 5.5l4.5 4.5m4 4l4.5 4.5m0-13L14 10m-4 4L5.5 18.5" />
						<circle cx="12" cy="3" r="1.5" />
						<circle cx="12" cy="21" r="1.5" />
						<circle cx="3" cy="12" r="1.5" />
						<circle cx="21" cy="12" r="1.5" />
						<circle cx="5.5" cy="5.5" r="1.5" />
						<circle cx="18.5" cy="18.5" r="1.5" />
						<circle cx="18.5" cy="5.5" r="1.5" />
						<circle cx="5.5" cy="18.5" r="1.5" />
					</svg>
				</div>
				<h3 class="text-lg md:text-xl font-bold text-brand-green mb-3 group-hover:text-brand-red transition-colors font-outfit">
					A Growing Network of Trusted Care
				</h3>
				<p class="text-sm text-brand-muted leading-relaxed font-sans">
					Expanding access to advanced, consistent healthcare through a connected and trusted care network.
				</p>
			</div>

		</div>
	</div>
</section>
